...
def take_best_strategy(df):
    ...
    for index, row in df.iterrows():
        # no candlestick pattern found
        if len(row[candle_names]) - sum(row[candle_names] == 0) == 0:
            continue
        if len(row[candle_names]) - sum(row[candle_names] == 0) == 1:
    ...
    return process_df_into_buys(df)


def process_df_into_buys(df):
    buy_list = []
    for k, row in df.iterrows():
        if 'Bull' in str(row['candlestick_pattern']):
            buy_list.append(1)
        else:
            buy_list.append(0)
    return buy_list
